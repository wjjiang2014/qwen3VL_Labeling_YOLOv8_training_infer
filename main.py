...
class Qwen3VLAnnotator:
    ...
    def build_prompt(self, classes: list) -> str:
        ...
        """
        # 将类别列表转为带引号的字符串
        class_str = ", ".join([c for c in classes])

        prompt = PROMPT_JSON["draw_box"].format(class_names=class_str)
        logger.info(prompt)
        return prompt
    ...
    def call_qwen3vl(self, image_path: str, prompt: str) -> dict:
        ...
        payload = {
            ...
            "stream": False,  # 关闭流式输出
            "options": {
                "num_ctx_tokens": 10240  # 上下文长度，视模型能力调整
            }
        }
    ...
    def denormalize_bbox(self, bbox: list) -> list:
        """
        反归一化边界框坐标
        ...
        try:
            ...
            for obj in qwen_result:
                ...
                if len(bbox) != 4:
                    continue
                # 反归一化坐标
                x1, y1, x2, y2 = self.denormalize_bbox(bbox)
    ...
    def verify_and_crop(self, image_path: str, label_path: str,
        ...
        try:
            ...
            for shape in label_data.get('shapes', []):
                ...
                # 获取边界框坐标
                x1, y1 = int(points[0][0] * width), int(points[0][1] * height)
                x2, y2 = int(points[1][0] * width), int(points[1][1] * height)
                ...
                # 4. 重新验证 - 构建验证 prompt
                # 简化验证：直接问是否为指定类别
                prompt = PROMPT_JSON["self_check"].format(class_names=classes)
                logger.info(prompt)
...
class YOLOTrainer:
    ...
    def convert_labelme_to_yolo(self, labelme_dir: str, output_dir: str,
        ...
        # 注意: 使用绝对路径，因为 Ultralytics 会根据 settings.yaml 中的 datasets_dir 拼接相对路径
        train_path = os.path.abspath(os.path.join(output_dir, 'images', 'train'))
        val_path = os.path.abspath(os.path.join(output_dir, 'images', 'train'))
...
"""
    ...
    def train(self, data_yaml: str, epochs: int = 100,
        ...
        # 返回最佳模型路径
        best_model = os.path.join('runs', 'train', 'weights', 'best.pt')
...
class Pipeline:
    ...
    def __init__(self, config: dict):
        ...
        self.labelme_dir = os.path.join(self.base_dir, f"labelme_output")   #_{timestamp}
        self.verified_dir = os.path.join(self.base_dir, f"verified_output")
        self.crop_dir = os.path.join(self.base_dir, f"crop_verify")
        self.yolo_dir = os.path.join(self.base_dir, f"yolo_dataset")
        self.infer_dir = os.path.join(self.base_dir, f"inference_output")
        ...
        self.annotator = Qwen3VLAnnotator(
            ollama_url=config.get('ollama_url', config.get("ollama_url", 'http://localhost:11434/api/generate')),
            model_name=config.get('qwen_model', config.get("qwen_model", "qwen3-vl:30b"))
        )
...
示例:
  ...
  python main.py -c config.json -i /data/images -e 50
    ...
    parser.add_argument(
        '--image-dir', '-i',
        type=str,
        default="/data6/xu/VL/scripts/det_demo/visual_vehicle_speed/train",
